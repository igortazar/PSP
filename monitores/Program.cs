...
namespace monitores
{
    ...
    {
        const int MagicNumber = 5;       // Indicates how many times to bounce between ping and pong threads 
        ...
        public void Consumer()
        {
            lock (m_lock)                                           // Allows only one thread at a time inside m_lock 
            {
                ...
                while (Monitor.Wait(m_lock))                  // Wait in the loop while producer is busy. Exit when producer times-out. 1000 = 1 second; app will hang without this time-out value 
                {
                    int data = m_queue.Dequeue();
                    Console.WriteLine("consumer {0}", data);
                    Monitor.Pulse(m_lock);              // Release consumer
                    if (data == MagicNumber-1)          // Fin del trabajo no queda nada por consumir
                        break;
                }
            }
        }
